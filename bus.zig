...
pub fn run() void {
    ...
    while (cycle_count <= 30000) : (cycle_count += 1) {
        if (cpu_state._current_activity == cpu.CpuActivity.FETCHING_INSTRUCTION) {
            std.debug.warn("{X:4}\tA:{X:2} X:{X:2} Y:{X:2} P:{X:2} -- {X:2} -- {}\n", .{ cpu_state.mem_address, cpu_state.regs.A, cpu_state.regs.X, cpu_state.regs.Y, @bitCast(u8, cpu_state.regs.P), read_memory(cpu_state.mem_address, internal_ram, cartridge_mapper), cycle_count + 1 });
        }
